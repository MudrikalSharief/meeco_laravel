        ...
        <div class="flex items-center justify-between bg-gradient-to-r from-indigo-600 to-blue-500 text-white shadow-md rounded-lg px-6 py-4 mb-6">
            ...
            <div class="flex items-center gap-4">
                <button id="viewToggleBtn" class="bg-blue-50 text-blue-600 px-3.5 py-1.5 rounded-full shadow-sm hover:shadow transition-all duration-200 flex items-center gap-2 font-medium border border-blue-100">
                    ...
                    <span id="viewToggleText">Group by Subject</span>
                </button>
            </div>
        </div>
            ...
            viewToggleBtn.addEventListener('click', function() {
                ...
                if (isGroupedView) {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            subjectsContainer.classList.add('opacity-100');
                            subjectsContainer.classList.remove('opacity-0');
                        }, 50);
                    }, 200);
                    ...
                    // Load reviewers grouped by subject
                    loadReviewersBySubject();
                } else {
                    // Animate the transition
                    subjectsContainer.classList.add('opacity-0');
                    setTimeout(() => {
                    ...
                    viewToggleText.textContent = 'Group by Subject';
            ...
            function loadReviewersBySubject() {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (data.reviewersBySubject.length === 0) {
                            subjectsContainer.innerHTML = '<div class="text-center py-4">No reviewers available</div>';
                            return;
                        }
                        
                        data.reviewersBySubject.forEach((item, index) => {
                            // Get color for this subject (cycle through colors)
                            const colorClass = subjectColors[index % subjectColors.length];
                            
                            // Create subject group container
                            let subjectGroupDiv = document.createElement('div');
                            subjectGroupDiv.classList.add(
                                'w-full',
                                'mb-6',
                                'rounded-lg',
                                'overflow-hidden',
                            ...
                            // Create subject header with gradient background
                            ...
                            colorClass.split(' ').forEach(cls => subjectDiv.classList.add(cls));
                            
                            // Add subject icon based on subject name
                            const iconSvg = getSubjectIcon(item.subject.name);
                            ...
                            subjectDiv.innerHTML = `
                                ...
                                <div class="text-sm opacity-80">${item.reviewers.length} ${item.reviewers.length === 1 ? 'reviewer' : 'reviewers'}</div>
                            `;
                            
                            subjectGroupDiv.appendChild(subjectDiv);
                            
                            // Create container for reviewers
                            let reviewersListContainer = document.createElement('div');
                            reviewersListContainer.classList.add('bg-white', 'py-1');
                            
                            // Add reviewers for this subject
                            ...
                            subjectGroupDiv.appendChild(reviewersListContainer);
                            subjectsContainer.appendChild(subjectGroupDiv);
                        });
                        
                        // Fade in the subjects container
                        setTimeout(() => {
                            subjectsContainer.classList.remove('opacity-0');
                        }, 100);
                    } else {
                        console.error('Failed to load reviewers by subject');
                    }
                })
                .catch(error => {
